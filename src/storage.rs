...
impl SecureStorage {
    pub fn new() -> Result<Self> {
        ...
        fs::create_dir_all(&base_path)
            .context("Failed to create ryzan config directory")?;
    ...
    pub fn save_config(&self, config: &WalletConfig) -> Result<()> {
        ...
        let temp_path = self.config_path.with_extension("tmp");
        {
            ...
            temp_file.write_all(config_data.as_bytes())
    ...
    pub fn save_wallet(&self, wallet: &SecureWallet, password: &str) -> Result<String> {
        ...
        let temp_path = wallet_path.with_extension("tmp");
        {
            ...
            temp_file.write_all(storage_json.as_bytes())
                .context("Failed to write wallet data")?;
            temp_file.sync_all()
                .context("Failed to sync wallet file")?;
        }
        
        fs::rename(&temp_path, &wallet_path)
    ...
    pub fn load_wallet(&self, wallet_filename: &str, password: &str) -> Result<SecureWallet> {
        ...
        if !wallet_path.exists() {
            return Err(anyhow::anyhow!("Wallet file not found: {}", wallet_filename));
        }
        
        let storage_json = fs::read_to_string(&wallet_path)
            .context("Failed to read wallet file")?;
        
        let storage_data: StorageFormat = serde_json::from_str(&storage_json)
            .context("Failed to parse wallet file")?;
        
        if storage_data.version != 1 {
            return Err(anyhow::anyhow!("Unsupported wallet file version: {}", storage_data.version));
        }
        ...
        let decrypted_wallet = storage_crypto.decrypt(&storage_data.encrypted_data, &storage_data.nonce)
