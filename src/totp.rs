...
impl TotpManager {
    ...
    pub fn verify_code(&self, input_code: &str, tolerance_windows: u8) -> Result<bool> {
        ...
        for window_offset in -(tolerance_windows as i32)..=(tolerance_windows as i32) {
            ...
            let expected_code_str =
                totp_custom::<Sha1>(DEFAULT_STEP, 6, &self.secret, check_timestamp);
    ...
    pub fn generate_setup_qr(&self) -> Result<String> {
        ...
        let qr_code = QrCode::new(&totp_url).context("Failed to generate QR code")?;
...
pub fn display_totp_qr(totp_manager: &TotpManager) -> Result<()> {
    ...
    println!(
        "\x1b[38;5;240m│\x1b[90m Account:\x1b[0m {:<50} \x1b[38;5;240m│\x1b[0m",
        account_info
    );
    println!(
        "\x1b[38;5;240m│\x1b[90m Secret:\x1b[0m  {:<50} \x1b[38;5;240m│\x1b[0m",
        secret_b32
    );
    println!(
        "\x1b[38;5;240m└─────────────────────────────────────────────────────────────┘\x1b[0m"
...
mod tests {
    ...
    fn test_totp_with_known_values() {
        ...
        let code = totp_manager.generate_current_code().unwrap();
        assert!(
            totp_manager.verify_code(&code, 2).unwrap(),
            "Should verify our own generated code"
        );
    }
