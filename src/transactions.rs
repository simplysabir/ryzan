    ...
    pub async fn send_sol(
        ...
        if use_ephemeral {
            ...
            if ephemeral_balance < lamports {
                ...
                self.fund_ephemeral_account(wallet, &sender_keypair, lamports + 5000).await?;
            }
        }
        ...
        let mut instructions = vec![
            system_instruction::transfer(&sender_pubkey, &recipient, lamports)
        ];
...
fn extract_memo_from_transaction(transaction: &solana_sdk::transaction::VersionedTransaction) -> Option<String> {
...
pub async fn process_batch_transactions(
    ...
    for tx in transactions {
        ...
        let signature = tx_manager.send_sol(
            wallet,
            recipient,
            lamports,
            tx.memo.as_deref(),
