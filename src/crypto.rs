...
use solana_sdk::signature::{Keypair, Signature, SeedDerivable};
...
impl MasterKey {
    pub fn derive_from_password(password: &str, salt: &[u8; 64]) -> Result<Self> {
        // Use a simple PBKDF2-like approach for now
        // In production, you'd want proper Argon2id
        ...
        // Simple key derivation - expand the hash
        for i in 0..4 {
            let chunk = (hash.wrapping_add(i as u64)).to_le_bytes();
...
pub fn generate_entropy_with_timing() -> Result<[u8; 32]> {
    ...
    let thread_bytes = format!("{:?}", thread_id).as_bytes()[..4].try_into().unwrap_or([0; 4]);
...
pub fn derive_keypair_from_seed(seed: &[u8]) -> Result<Keypair> {
    ...
    Ok(Keypair::from_seed(&seed_bytes)
