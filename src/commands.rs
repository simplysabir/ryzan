...
use crate::transactions::{TransactionManager, BatchTransaction, process_batch_transactions};
...
impl CommandHandler {
    ...
    pub async fn execute(&mut self, command: Commands) -> Result<()> {
        match command {
            ...
            Commands::Send { address, amount, totp, memo, ephemeral, token } => {
                self.send_transaction(address, amount, totp, memo, ephemeral, token).await
            }
            ...
            Commands::BatchSend { file, totp } => {
                self.batch_send(file, totp).await
            }
            Commands::Stake { validator, amount, totp } => {
                self.stake_sol(validator, amount, totp).await
            }
            Commands::ExportKey { totp, confirm } => {
                self.export_private_key(totp, confirm).await
            }
            Commands::SetLimit { daily, totp } => {
                self.set_daily_limit(daily, totp).await
            }
            Commands::History { limit, export } => {
                self.show_transaction_history(limit, export).await
            }
            Commands::Portfolio { tokens, staking } => {
                self.show_portfolio(tokens, staking).await
            }
            Commands::Swap { from, to, amount, totp } => {
                self.swap_tokens(from, to, amount, totp).await
            }
            Commands::Delete { name, totp, confirm } => {
                self.delete_wallet(name, totp, confirm).await
            }
            Commands::Update => {
                self.update_wallet().await
            }
            Commands::Panic { emergency_code } => {
                self.panic_wipe(emergency_code).await
            }
        }
    }
    ...
    async fn send_transaction(&mut self, address: String, amount: f64, totp_code: String, memo: Option<String>, ephemeral: bool, token: Option<String>) -> Result<()> {
        ...
        let signature = match token {
            Some(token_mint) => {
                ...
                print_operation_status(&format!("Sending {} tokens to {}", amount, address), "info");
                tx_manager.send_spl_token(wallet, recipient, mint_pubkey, token_amount, memo.as_deref(), ephemeral).await?
            }
            None => {
                // SOL transfer
                ...
                tx_manager.send_sol(wallet, recipient, lamports, memo.as_deref(), ephemeral).await?
            }
        };
    ...
    async fn recover_wallet(&mut self, file: String, name: Option<String>) -> Result<()> {
        ...
        let backup_data = std::fs::read_to_string(&file)
            .context("Failed to read backup file")?;
        
        let backup: crate::backup::WalletBackup = serde_json::from_str(&backup_data)
        ...
        let recovered_wallet = backup_manager.restore_from_backup(&backup)
    ...
    async fn batch_send(&mut self, file: String, totp: String) -> Result<()> {
        ...
        let batch_data = std::fs::read_to_string(&file)
            .context("Failed to read batch transaction file")?;
        
        let transactions: Vec<BatchTransaction> = serde_json::from_str(&batch_data)
            .context("Failed to parse batch transaction file")?;
        ...
        for (i, tx) in transactions.iter().enumerate() {
            println!("  {}. {} SOL to {} {}", 
                ...
                tx.amount, 
                tx.recipient,
                tx.memo.as_ref().map(|m| format!("({})", m)).unwrap_or_default()
            );
        }
        ...
        print_operation_status(&format!("Batch send completed! {} transactions sent", signatures.len()), "success");
    ...
    async fn stake_sol(&mut self, validator: String, amount: f64, totp: String) -> Result<()> {
        ...
        let tx_manager = TransactionManager::new()?;
        let signature = tx_manager.stake_sol(wallet, validator_pubkey, lamports).await?;
        
        print_operation_status(&format!("Staking successful! Signature: {}", signature), "success");
    ...
    async fn export_private_key(&mut self, totp: String, confirm: String) -> Result<()> {
        ...
        if confirm != "YES" {
            return Err(anyhow::anyhow!("Export cancelled. You must type 'YES' exactly to confirm."));
        }
        ...
        println!("\x1b[38;5;196m├─────────────────────────────────────────────────────────────┤\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ EXPOSING YOUR PRIVATE KEY IS EXTREMELY DANGEROUS         │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ ANYONE WITH THIS KEY CAN STEAL ALL YOUR FUNDS            │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ NEVER SHARE THIS KEY OR STORE IT ONLINE                  │\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m└─────────────────────────────────────────────────────────────┘\x1b[0m");
        println!();
        ...
        print_operation_status("Private key exported! Clear your terminal history immediately!", "warning");
        Ok(())
    }
    ...
    async fn show_transaction_history(&mut self, limit: u32, export: Option<String>) -> Result<()> {
        ...
        let tx_manager = TransactionManager::new()?;
        let history = tx_manager.get_transaction_history(wallet, limit as usize).await?;
    ...
    async fn swap_tokens(&mut self, from: String, to: String, amount: f64, totp: String) -> Result<()> {
        let _wallet = self.current_wallet.as_ref()
            .ok_or_else(|| anyhow::anyhow!("No wallet unlocked. Use 'ryzan unlock --name <wallet>' first"))?;
        
        // Verify TOTP
        let totp_manager = TotpManager::new(
            _wallet.totp_secret.clone(),
            _wallet.name.clone()
        );
    ...
    async fn delete_wallet(&mut self, name: String, totp: String, confirm: String) -> Result<()> {
        // Security check - require exact confirmation
        if confirm != "DELETE" {
            return Err(anyhow::anyhow!("Deletion cancelled. You must type 'DELETE' exactly to confirm."));
        }
        ...
        println!("\x1b[38;5;196m├─────────────────────────────────────────────────────────────┤\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ YOU ARE ABOUT TO PERMANENTLY DELETE WALLET '{:<10}' │\x1b[0m", name);
        println!("\x1b[38;5;196m│ ❌ THIS ACTION CANNOT BE UNDONE                             │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ ALL FUNDS WILL BE LOST UNLESS YOU HAVE BACKUPS          │\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m└─────────────────────────────────────────────────────────────┘\x1b[0m");
        println!();
        ...
        print_operation_status(&format!("Wallet '{}' has been permanently deleted", name), "success");
        Ok(())
    }
    ...
    async fn panic_wipe(&mut self, emergency_code: String) -> Result<()> {
        println!();
        println!("\x1b[38;5;196m┌─────────────────────────────────────────────────────────────┐\x1b[0m");
        println!("\x1b[38;5;196m│                       🚨 PANIC MODE 🚨                       │\x1b[0m");
        println!("\x1b[38;5;196m├─────────────────────────────────────────────────────────────┤\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ THIS WILL PERMANENTLY DELETE ALL WALLET DATA            │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ ALL WALLETS AND SETTINGS WILL BE WIPED                  │\x1b[0m");
        println!("\x1b[38;5;196m│ ❌ THIS ACTION CANNOT BE UNDONE                             │\x1b[0m");
        println!("\x1b[38;5;196m│                                                             │\x1b[0m");
        println!("\x1b[38;5;196m└─────────────────────────────────────────────────────────────┘\x1b[0m");
        println!();
        ...
        if emergency_code != expected_code {
            return Err(anyhow::anyhow!("Invalid emergency code. Panic wipe cancelled."));
        }
        ...
        print_operation_status("Emergency wipe completed! All wallet data has been destroyed.", "success");
        println!();
