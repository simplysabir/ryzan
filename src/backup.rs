...
impl BackupManager {
    ...
    pub fn generate_backup_qr_code(&self, wallet: &SecureWallet) -> Result<String> {
        ...
        let qr_code = QrCode::new(&qr_data)
            .context("Failed to generate QR code for backup")?;
    ...
    pub fn save_backup_to_file(&self, backup: &WalletBackup, file_path: &Path) -> Result<()> {
        ...
        fs::write(file_path, backup_json)
    ...
    pub fn restore_from_backup(&self, backup: &WalletBackup) -> Result<SecureWallet> {
        ...
        if calculated_checksum != backup.checksum {
            return Err(anyhow::anyhow!("Backup integrity check failed - corrupted data"));
        }
    ...
    pub fn restore_from_qr_data(&self, qr_data: &str) -> Result<SecureWallet> {
        ...
        let minimal_backup: MinimalBackup = serde_json::from_str(&backup_json)
            .context("Failed to parse minimal backup")?;
...
impl PaperBackup {
    pub fn print(&self) {
        ...
        println!("│ Created: {:<51} │", self.creation_date.format("%Y-%m-%d %H:%M:%S UTC"));
        println!("│ Backed up: {:<49} │", self.backup_date.format("%Y-%m-%d %H:%M:%S UTC"));
        println!("│                                                             │");
        println!("└─────────────────────────────────────────────────────────────┘");
        println!();
